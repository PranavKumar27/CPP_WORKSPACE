...
void ProcessMessage(int& clientSession)
{
    char buff[256+1] = {0};
    int nRet = recv(clientSession,buff,256,0);
    //if(nRet<0)
    {
    ...
    //else
    {
       cout  << "Message Recev from client=" << buff;
       send(clientSession,"Processed Your Message",23,0);
       cout << "~~~~~" << endl;
    }
}
